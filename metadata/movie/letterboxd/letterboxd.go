...
// WithBaseURL overrides the default Letterboxd API base URL.
...
func (c *Client) get(ctx context.Context, path string, dst any) error {
	...
	if err != nil {
		return fmt.Errorf("letterboxd: parse URL: %w", err)
	}

	req, err := http.NewRequestWithContext(ctx, http.MethodGet, u.String(), http.NoBody)
	if err != nil {
		return fmt.Errorf("letterboxd: create request: %w", err)
	}
	...
	if err != nil {
		return fmt.Errorf("letterboxd: read response: %w", err)
	}
...
func (c *Client) doJSON(ctx context.Context, method, path string, payload, dst any) error {
	...
	resp, err := c.httpClient.Do(req)
	if err != nil {
		return fmt.Errorf("letterboxd: %s %s: %w", method, path, err)
	}
	defer resp.Body.Close()
